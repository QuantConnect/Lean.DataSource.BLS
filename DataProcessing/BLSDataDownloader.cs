...
{
    /// <summary>
    /// BLSUniverseDataDownloader implementation.
    /// </summary>
    public class BLSUniverseDataDownloader : IDisposable
    {
        ...
        public BLSUniverseDataDownloader(string destinationFolder, string apiKey = null)
        {
        ...
        public bool Run()
        {
            ...
            Log.Trace($"BLSUniverseDataDownloader.Run(): Finished in {stopwatch.Elapsed.ToStringInvariant(null)}");
            return true;
        }
            ...
            for (var retries = 1; retries <= _maxRetries; retries++)
            {
                try
                {
                    using (var client = new HttpClient())
                    {
                        ...
                        {
                            Log.Error($"BLSUniverseDataDownloader.HttpRequester(): Files not found at url: {Uri.EscapeUriString(url)}");
                ...
                {
                    Log.Error(e, $"BLSUniverseDataDownloader.HttpRequester(): Error at HttpRequester. (retry {retries}/{_maxRetries})");
                    Thread.Sleep(1000);
                }
            }
