...
namespace QuantConnect.DataProcessing
{
    ...
    public class Program
    {
        ...
        public static void Main()
        {
            ...
            BLSUniverseDataDownloader instance = null;
            try
            {
                // Pass in the values we got from the configuration into the downloader/converter.
                instance = new BLSUniverseDataDownloader(destinationDirectory);
            }
            catch (Exception err)
            {
                Log.Error(err, $"QuantConnect.DataProcessing.Program.Main(): The downloader/converter for {BLSUniverseDataDownloader.VendorDataName} {BLSUniverseDataDownloader.VendorDataName} data failed to be constructed");
                Environment.Exit(1);
            }
            ...
            try
            {
                ...
                if (!success)
                {
                    Log.Error($"QuantConnect.DataProcessing.Program.Main(): Failed to download/process {BLSUniverseDataDownloader.VendorName} {BLSUniverseDataDownloader.VendorDataName} data");
                    Environment.Exit(1);
                }
            }
            catch (Exception err)
            {
                Log.Error(err, $"QuantConnect.DataProcessing.Program.Main(): The downloader/converter for {BLSUniverseDataDownloader.VendorDataName} {BLSUniverseDataDownloader.VendorDataName} data exited unexpectedly");
                Environment.Exit(1);
            }
            finally
            {
